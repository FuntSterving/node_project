...
const token = "6364387307:AAFRzDgepNBbD1LETCQAU_i96dvreu88VNM";
const webAppUrl = "https://reactprojectstep.netlify.app";
...
bot.on("message", async (msg) => {
  ...
  if (text === "/start") {
    await bot.sendMessage(chatId, "Купишь вертолёт?", {
      reply_markup: {
        inline_keyboard: [
          [{ text: "Оставить свои данные", web_app: { url: webAppUrl + "/form" } }],
          [{ text: "Посмотреть вертолёт", web_app: { url: webAppUrl } }],
        ],
      },
    });
  ...
  try {
    await bot.answerWebAppQuery(queryId, {
      ...
      input_message_content: {
        message_text: `Поздравляю с покупкой, вы приобрели курсов на ${totalPrice}`,
      },
    });
